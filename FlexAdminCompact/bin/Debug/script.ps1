...
# an flexadmin.config file:
#
#   <Step ScheduleScriptTarget="ScriptToBeExecuted"
...
#
# ScheduleFlexAdminTarget: Name of the flexadmin target to be executed
# by the scheduled task. This can include
# config settings. e.g.
# "$(MyRootFolder)\SubFolder\myscript.ps1"
#
# TaskName: Name of the scheduled task to configure. This can include
# config settings. e.g.
# "My Folder\$(TenantUID)\Name of the scheduled task"
...
function DoScheduleScriptTargetStep($step, $target, $moduleConfig)
{
  
  $scheduledTarget = $($step.ScheduledScriptTarget)
  $scriptPath = (ReplaceConfigValueReferences $scheduledTarget)

  $logFile = $($step.LogFile)
  $logPath = (ReplaceConfigValueReferences $logFile)

  $args = $step.CommandlineArguments
  $taskName = (ReplaceConfigValueReferences $step.TaskName)
  $scheduleOptions = $step.ScheduleOptions -split " "
  
  $commandLine = "cmd /C powershell.exe -File `"$scriptPath`" $args > `"$logPath`" 2>&1"

  Log "Configuring scheduled task '$taskName' to run a target script $scheduledTarget"
  Log "Scheduled task command line: $commandLine"
  ...
  foreach ($mc in $moduleConfigs) {
    foreach ($targetNode in $mc.Xml.Configuration.Target | ?{$_.Name -eq $target}) {
      ...
      foreach ($s in $targetNode.Step) {
        ...
        if ($s.ScheduledScriptTarget) {
          $currentStepNumber.Value++
          Log "Executing step $($currentStepNumber.Value) (schedule script target $($s.ScheduledScriptTarget))"
          ...
          elseif (!(DoScheduleScriptTargetStep $s $target $mc)) {
            return $false
          }
        }
