...
###########################################################################
# Get the FNMSAdmins security group (or create it if it does not exist)

function GetFNMSAdminsGroup
{
	$fnmsAdminsGroup = (GetConfigValue "FNMSAdminsGroup")

	$group = CreateGlobalSecurityGroup $fnmsAdminsGroup "Members of this group can configure FlexNet Manager Suite"
	if (!$group) {
		return $null
	}
	...
	try {
		...
		Log "Verifying that $fnmsAdminsGroup is a member of $localAdminsGroupName"
		$admins = [ADSI]"WinNT://$(hostname)/$($localAdminsGroupName.Split("\")[1])"

		$isMember = $false
		$admins.Invoke("Members") | %{
			$isMember = $isMember -or $_.GetType().InvokeMember("Name", "GetProperty", $null, $_, $null) -eq $group.SamAccountName
		}
	} 
	catch {
		Log "ERROR: Unable to determine whether $fnmsAdminsGroup is a member of $($localAdminsGroupName): $_" -level Error
		return $null
	}
	
	try {
		if (!$isMember) {
			Log "Group $fnmsAdminsGroup is not a member of the $($localAdminsGroupName); adding membership"
			$result = $admins.Add("WinNT://$($fnmsAdminsGroup -replace "\\", "/")")
			Log "`tGroup membership added"
		}
	}
	catch {
		Log "ERROR: Unable to add $fnmsAdminsGroup to $($localAdminsGroupName): $_" -level Error
		return $null
	}
