...
###########################################################################
# Validate that the $settingName parameter has been provided in the 
# command line

function ValidateGlobalCommandLineSetting([string]$settingName)
{
...
function AddPrivilege([string]$account, [string]$right)
{
	[MyLsaWrapper.LsaWrapperCaller]::AddPrivileges("$account", "$right")
  Write-Host "AddPrivilege -account $account -right $right success"	
}
...
{  
	Start-Job `
      ...
      $coderef = Join-Path $scriptPath "LSAWrapper.cs"     
			Add-Type -Path $coderef
      Write-Host "Add-Type $coderef success"
...
###########################################################################
# Check if stored credentials for Target exist

function StoredCredentialsExist([string]$Target, [switch]$showPassword)
{   
  # initial for credentials registration
  Log ""  
  if ( [StoredCredential]::Exists( $Target ) ) {
    Log "Credential for '$Target' found in Credential Manager." 
    Log ""    
  } else {
