...
if [ -f "$BASEDIR/Bootstrap Failover Settings.osd" ]; then
	echo
	echo "Installing bootstrap failover settings"
	/opt/managesoft/bin/ndlaunch -r "file://`cd $BASEDIR && pwd`/Bootstrap Failover Settings.osd" -o PkgType=ClientSettings || {
		   ret=1
		   echo "ERROR: Failed to install bootstrap failover settings details (inspect /var/opt/managesoft/log/installation.log for details)" 1>&2
	}
fi
