...
###########################################################################
# Configure IIS Features For Analytics
...
function InstallAnalytics
{
	...
	try {
		...
		try {
		  $installerEXE = GetConfigValue "AnalyticsInstaller"
			$installer = Join-Path (GetConfigValue "AnalyticsInstallerDir") $installerEXE

		  $found = Test-Path $installer -pathType Leaf
      if (!$found) 
      {
		    Log ""
		    Log "Analytics Installer file not found: $installer" -level Error
		    Log ""
			...
			& .\InstallCognos.ps1 -CognosInstallerFilePath $installer | Log
		}
		catch {
			Log "" -level Error
...
###########################################################################
# Set FNMS Cognos Config Answers
