...
function SetConfigValue([string]$settingName, [string]$value)
{
  ...
  #we are restricted to using HKCU when security policy has been set to block HKLM writes
  #Set-ItemProperty -path $RegSettingsKey -name $settingName -value $value #-ErrorAction Ignore    
...
if ([string]::IsNullOrEmpty($FlexAdminPath))
{
