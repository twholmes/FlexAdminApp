...
{
	Log "Configuring drivers for Excel 2007 spreadsheets"

	$key = "HKLM:\SOFTWARE\Microsoft\Office\14.0\Access Connectivity Engine\Engines\Excel"
	if ((Configure-ExcelDrivers $key) -eq $false)
	{
		Log "" -level Error
		Log "ERROR: Could not configure drivers for Excel 2007 spreadsheets by configuring values under $key" -level Error
	...
	Log "`tDrivers for Excel 2007 spreadsheets configured"
