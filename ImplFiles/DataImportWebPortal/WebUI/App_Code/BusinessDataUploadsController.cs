...
public class BusinessDataUploadsController: System.Web.Mvc.Controller
{
  ...
  // ////
  // Flexera has switched to using native DevExpress code in FNFL 2020-R2
  // ////
  #if FNMS2020R2
  
	private void SaveUploadedFileUsingDX(BusinessAdapterRegistration adapter, DevExpress.Web.UploadedFile uploadedFile)
	{
		...
		mLogger.Info("Saving uploaded file {0}", uploadedFile.FileName);		
		string savedFile = System.IO.Path.Combine(dir, uploadedFile.FileName);
		
		mLogger.Info("Saving file uploaded by {0} to {1}", ManageSoft.Identity.SystemIdentity.UserName, savedFile);
		uploadedFile.SaveAs(savedFile);
		...
		BusinessDataDatabaseLayer.ExecuteStoredProcedure(
			...
			new System.Collections.Generic.Dictionary<string, object>{
				...
				{ "@DataSourceFileName", uploadedFile.FileName },
				{ "@OperatorLogin", ManageSoft.Identity.SystemIdentity.UserName },
			}
		);
  ...
  // this is the pre FNMS2020-R2 function
    ...
    #if !FNMS2020R2
      // FNMS2020-R1 and earlier handler
	    ...
	    // FNMS2020-R2 handler
			Flexera.Web.Core.Display.FileUploadHelper.FileUploadHandler(
				"businessDataUploadControl", new string[] { allowedExtension },
				maximumFileSize: 20 * 1024 * 1024,
				uploadCompleteHandler: delegate(DevExpress.Web.UploadedFile uploadedFile) {
					SaveUploadedFileUsingDX(r, uploadedFile);
					return string.Empty;
				}
			);
    #endif			
