...
$FlexAdminPath = Split-Path $MyInvocation.MyCommand.Path
...
function DoScheduleFlexAdminTargetStep($step, $target, $moduleConfig)
{
	...
	$commandLine = "`"$PSHOME\PowerShell.exe`" $psArgs -Command `"& {'$FlexAdminPath\flexadmin.ps1' $scheduledTarget -quiet $settings}`""
