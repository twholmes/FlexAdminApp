...
Registry: set; root = HKLM; key = "SOFTWARE\\ManageSoft Corp\\ManageSoft\\Common\\DownloadSettings\\Bootstrap DL $i"; name = Priority; value = ""; type = SZ
"@
		}
...
Registry: set; root = HKLM; key = "SOFTWARE\\ManageSoft Corp\\ManageSoft\\Common\\DownloadSettings\\Bootstrap DL $i"; name = Priority; value = "100"; type = SZ
"@
		}
	} |
...
function PrepareWindowsInvAgentInstallers
{
	...
	$sourcePath = Join-Path (GetFNMSInstallDir) "WebUI\Content\Packages\Flexera\Adoption\*\Rev1.0\FlexNet Inventory Agent\managesoft-*.zip"
	$sourceZips = Get-Item $sourcePath -ErrorAction SilentlyContinue

	if (!$sourceZips) {
		Log "ERROR: No Windows FlexNet inventory agent package directories found at '$sourcePath'" -level "Error"
		return $false
	}

	# Files to extract from each $sourceZips file
	$versionedFiles = (
	...
	foreach ($sourceZip in $sourceZips) {
		$version = $sourceZip -replace ".*\\Adoption\\(.*)\\Rev1.0\\.*", "`$1"
		...
		Log ""
		Log "`tExtracting files from $($sourceZip.FullName):"

		$archive = New-Object System.IO.Compression.ZipArchive(New-Object System.IO.FileStream($sourceZip.FullName, "Open", "Read"))

		try {
			foreach ($f in $versionedFiles) {
				Log "`t`t$f"
				[System.IO.Compression.ZipFileExtensions]::ExtractToFile($archive.GetEntry($f), (Join-Path $targetDir $f), $true)
			}
		}
		finally {
			$archive.Dispose();
...
function PrepareUNIXInvAgentInstallers
{
	...
	$sourcePath = Join-Path (GetFNMSInstallDir) "WebUI\Content\Packages\Flexera\Adoption\*\Rev1.0\ManageSoft for * managed devices"
